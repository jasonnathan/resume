  ...
  init: function(withRipple) {
    ...
    var rippleStart = function(e, target, callback) {
      ...
      var $rippleWrapper = target,
        ...
        mousePos = {
          x: e.clientX - elPos.left,
          y: ((window.ontouchstart) ? e.clientY - window.scrollY : e.clientY) - elPos.top
        },
        scale = "scale(" + Math.round($rippleWrapper.offsetWidth / 5) + ")",
        rippleEnd = new CustomEvent("rippleEnd", {
          detail: $ripple
        }),
        _rippleOpacity = 0.3,
        refreshElementStyle;
      ...
      if (e.touches) {
        mousePos = {
          x: e.touches[0].clientX - elPos.left,
          y: e.touches[0].clientY - elPos.top
        };
      }
